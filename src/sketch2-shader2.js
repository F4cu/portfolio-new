...
const fragShader = `
  ...
  void main() {
      ...
      // Film grain (finer + more organic)
    float grain1 = random(uv * vec2(800.0, 600.0) + time * 10.0);
    float grain2 = random(uv * vec2(1200.0, 900.0) - time * 5.0);

    // Combine + center around 0
    float grain = (grain1 + grain2 - 1.0);

    // Control intensity via existing uniform
    grain *= noiseAmount;
...
// Dynamic extrusion state (can change during glitch phases)
let currentExtrusion = {
  depth: extrusionDepth,
  stroke: strokeThickness,
};
// Line opacity state (for startup fade-in)
let currentLineOpacity = 1.0;
  ...
  const sketch = (p) => {
    ...
    function updateGlitchState(now) {
        ...
        if (initialGlitchActive) {
            ...
            // Fade in line opacity from 0 → 1 during startup
            currentLineOpacity = easeProgress;
            ...
            currentUniforms.chromaOffset = INIT.chromaOffset;

            // Animate extrusion during initial glitch
            currentExtrusion.depth =
              p.lerp(1000, extrusionDepth, easeProgress) +
              (Math.random() - 0.5) * 20 * (1 - easeProgress);
            currentExtrusion.stroke = p.lerp(2, strokeThickness, easeProgress);
            ...
            if (progress >= 1.0) {
                ...
                // Reset extrusion to default after initial glitch
                currentExtrusion.depth = extrusionDepth;
                currentExtrusion.stroke = strokeThickness;
    ...
    p.draw = () => {
      ...
      graphics.push();
      const floorCol = p.color(floorStrokeColor);
      floorCol.setAlpha(currentLineOpacity * 255);
      graphics.stroke(floorCol);
      ...
      strokeCol.setAlpha(currentLineOpacity * 255);
      graphics.stroke(strokeCol);
      graphics.strokeWeight(currentExtrusion.stroke);
      ...
      const depth = currentExtrusion.depth;
